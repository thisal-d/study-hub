...
## Product Manager MERN Application Enhancement and Deployment

---

## 1. Lab Test Overview

You are provided with a starter MERN **Product Manager** project containing separate frontend and backend applications. Your task is to complete the given project, extend it by adding a new field called **Stock Quantity**, test it locally using your own MongoDB connection string, publish the source code to a public GitHub repository, and deploy the frontend and backend separately using free hosting platforms.
...
- Updating both frontend and backend to support a new numeric data field with conditional rendering.
...
- Frontend project of the Product Manager application (React + Vite).
- Backend project of the Product Manager application (Node.js + Express + MongoDB).
...
- Verify the following:
  ...
  - You can navigate between pages (Home, Add Product).
  - Existing features (Add Product / View Products / Edit Product / Delete Product) are working.
...
Modify the application by adding a new field named **Stock Quantity** to the Add Product form.
...
- Ensure that the **Stock Quantity** value is shown on the Home page together with the other product details (Name, Brand, Price, Description).
- Make sure the new field behaves correctly during **add**, **read**, and **update** operations.
- **Conditional Rendering:** Display the stock quantity with different styles or text based on the value:
  - If stock > 10, display the number normally (e.g., "In Stock: 15").
  - If stock is between 1 and 10, display a warning (e.g., "Low Stock: 5" in orange/red).
  - If stock is 0, display "Out of Stock" (in red).
...
- Screenshot of the locally running **updated** project — Add Product page *(showing the Stock Quantity field)*
- Screenshot of the locally running **updated** project — Home page *(showing Stock Quantity and conditional formatting)*
- Frontend Live URL
- Backend Live URL
...
| New field integration (Stock Quantity) | Adds the field to the form and updates frontend and backend correctly. | 15 | This includes schema and state updates. |
| Conditional Rendering logic | Correctly implements the logic to display "Out of Stock" or "Low Stock". | 15 | Must visually distinguish stock levels. |
